...
**題目（中文）**：中文法律情境敘述之多標籤文本分類 — 從案例故事預測法律議題
**題目（英文）**：Chinese Legal Scenario Classification — Multi-label Prediction of Legal Issues from Case Narratives
**課程**：輔大資管三 自然語言處理（王冠云老師）
**文件日期**：2026-04-17（v3：pivot 到 Text Classification 主軸）
**作者**：roy422
...
**v3 聚焦單一明確任務**：**多標籤法律議題分類**，直接對應 guideline §2.1 的 **Text Classification** 主題。
...
- **次要**：法律科普平台的初步分流（案件路由）、法扶諮詢專線前置分類、法學院學生自我檢核

### 1.3 使用者故事
> 使用者：「我被告妨害名譽 但是 threads 上的人是冒名的 怎麼辦」
> 系統：`[妨害名譽, 冒名/身分盜用, 個資侵害]`
> 並附上：建議查閱 刑法 §310 誹謗罪、刑法 §313 妨害信用、個資法 §41
...
- **用途**：議題 ontology 對應的法條原文查詢、Demo 顯示條文原文
- **授權**：CC BY 4.0

### 3.4 不使用（附理由）
- `tw-legal-nlp` (171 筆)：太小、`task` 欄位實際皆 None
...
- **標籤空間**：**30–50 個議題類別**（Step 1 從資料驅動產出，最終數字以 Step 1 實跑結果為準）
...
### Step 2 — 自動打標籤
**做什麼**：
1. 對每筆 row 跑 regex 從 assistant content 抽 (法規, 條號)
2. 查 ontology 映射到議題 ID
3. 一筆可有多 label
4. 丟掉：無標籤 / assistant 表示「資訊不足」 / user 句過短
...
- [ ] Rule-based / TF-IDF+SVM / BERT 三法端對端實作
- [ ] 量化比較表（in-domain + OOD aigrant）+ 錯誤分析
- [ ] Demo notebook（現場 demo 用）
- [ ] 3–6 頁 PDF 報告 + 10-12 分鐘 slides
...
| Written report 3–6 頁 PDF | `report/report.pdf` |
| Oral presentation 10–12 分鐘 | `report/slides.pdf` + `demo.ipynb` 現場跑 |
...
| Practical Applicability 30% | 明確使用者（遇法律問題的一般人）+ 真實資料（法院判決衍生）+ 具體應用（諮詢分流 / 法扶路由 / 法學自檢）|
